...
export default function ReviewQueuePage() {
  ...
  const handleAction = async (id: string, action: 'approve' | 'reject') => {
    try {
      if (action === 'approve') {
        ...
        toast({ title: "Approved", description: "Content has been moved to Approved state." });
      } else {
        // Mock reject logic
        toast({ title: "Rejected", description: "Content has been returned to Draft." });
      }
      loadData();
    } catch (e: any) {
  ...
  if (isLoading) return <div className="py-20 text-center text-muted-foreground">Accessing review queue...</div>;

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Compliance Review Queue</h1>
        <p className="text-muted-foreground">High-priority approvals for institutional content and governance updates.</p>
      </div>
      ...
      {pendingPages.length === 0 ? (
        <Card className="bg-muted/20 border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-muted-foreground max-w-xs">All submitted changes have been reviewed and processed.</p>
          </CardContent>
        </Card>
      ) : (
        <Card>
              ...
              <TableBody>
                {pendingPages.map((page) => (
                  <TableRow key={page.id}>
                    ...
                    <TableCell>
                      <code className="text-xs bg-muted px-1.5 py-0.5 rounded">v{page.currentVersion}.1-draft</code>
                    </TableCell>
                    <TableCell>
      ...
      {userRole !== 'ComplianceOfficer' && userRole !== 'SuperAdmin' && (
        <div className="p-4 bg-amber-500/10 border border-amber-500/50 rounded-lg flex items-start gap-3">
          ...
          <div className="text-xs text-amber-200/80">
            ...
            <p>You are viewing the queue as {userRole}. Only users with the <strong>ComplianceOfficer</strong> role can execute approval actions. Interface buttons are for simulation only.</p>
          </div>
        </div>
      )}
    </div>
