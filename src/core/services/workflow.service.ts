...
export const workflowService = {
  ...
  handleTransition: async (
    ...
    // Check Permissions
    const canManage = await permissionService.hasPermission(role, module, 'manage');
    const isCompliance = role === 'ComplianceOfficer' || role === 'SuperAdmin';
    
    if (targetStatus === 'Approved' || targetStatus === 'Rejected') {
      if (!isCompliance) throw new Error("Only Compliance Officers can approve or reject changes.");
    }
