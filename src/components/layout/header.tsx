          ...
          <div className="flex items-center gap-1.5 sm:gap-4">
            ...
            <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className="lg:hidden h-9 w-9 sm:h-10 sm:w-10" aria-label="Open Mobile Menu">
      ...
      return (
        <DropdownMenu key={item.id}>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="text-muted-foreground hover:text-foreground text-sm font-semibold flex items-center gap-1 px-3 h-9">
    ...
    return (
      <Link
        ...
        className={cn(
          ...
          isMobile ? "text-base hover:bg-muted hover:pl-4" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
        )}
      >
        {item.label}
      </Link>
  ...
  if (userRole !== 'public') {
    return (
      <div className={cn("flex items-center gap-2", isMobile && "flex-col w-full")}>
        <Button variant="outline" size="sm" onClick={() => { authService.signOut(); onAction?.(); }} className="gap-2 w-full font-bold">
  ...
  return (
    <div className={cn("flex items-center gap-2", isMobile && "flex-col w-full")}>
      <Button asChild variant="outline" size="sm" className="w-full font-bold">
        <Link href="/onboarding" onClick={onAction}>Start Onboarding</Link>
      </Button>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button size="sm" className="shadow-md w-full font-bold" aria-label="Institutional login simulator">Simulation Login</Button>
        </DropdownMenuTrigger>
